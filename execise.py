...
def implement_execute_tool_call(tool_call: Dict, available_tools: List[Tool]) -> str:
    ...
    if selected_tool:
        # Extract the 'text' argument from the args dictionary
        text_input = tool_args.get("text", tool_args)
        result = selected_tool.invoke(text_input)
        return str(result)
    else:
...
def implement_run_agent_with_tool_calling(
    ...
    if implement_check_for_tool_calls(response):
        # Step 3: Execute the tool call (take the first one)
        tool_call = response.tool_calls[0]
        result = implement_execute_tool_call(tool_call, available_tools)
        return result
    else:
        # Step 4: No tool calls, return the model's content
